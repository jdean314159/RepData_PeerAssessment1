...
# data_loc <- "C:/Users/jsdean/Documents/Reproducible_research/activity.csv"
 data_loc <- "/home/jsdean/Documents/R/Reproducible_research/activity.csv"
...
Mean   <- format(mean(steps, na.rm=TRUE), nsmall=2)
...
stats <- summarize(grouped, ave_steps=mean(steps, na.rm=TRUE))
...
data$interval <- as.factor(data$interval)
grouped <- group_by(data, interval)
t_means <- summarise(grouped, ave = mean(steps, na.rm=TRUE))
Max <- data[which.max(t_means$ave),]
interval <- as.character(Max$interval)
interval <- paste(substring(interval,1,1),":", substring(interval,2,3), sep="")
